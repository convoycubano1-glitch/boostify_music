    ...
    switch (decisionType) {
      case 'buy_token': {
        ...
        await db.insert(aiSocialPosts).values({
          artistId,
          contentType: 'thought',
          content: `Just invested in $${token[0].tokenSymbol}! 📈 I believe in this artist's vision and see huge potential. Supporting talent is what this community is about! #Investment #Web3Music`,
          status: 'published',
      ...
      case 'sponsor_collab': {
        ...
        await db.insert(aiSocialPosts).values({
          ...
          content: `🎵 Proud to sponsor the upcoming collab "${collab[0].title}"! Can't wait to see what these artists create together. Supporting our community! #Sponsor #Collaboration`,
          status: 'published',
