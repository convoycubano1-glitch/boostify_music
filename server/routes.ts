...
export function registerRoutes(app: Express): HttpServer {
  ...
  /**
   * Initialize Boostify Profile - Create default profile with demo songs
   */
  app.post('/api/init-boostify-profile', async (req, res) => {
    try {
      const { users, songs } = await import('./db/schema');
      ...
      for (const song of DEMO_SONGS) {
        await db.insert(songs).values({ ...song, userId });
      }

      return res.json({
        success: true,
        message: 'Boostify profile initialized successfully',
        profileUrl: '/artist/boostify',
        songsCount: DEMO_SONGS.length
      });
    } catch (error: any) {
      console.error('Error initializing Boostify profile:', error);
      return res.status(500).json({
        success: false,
        error: error.message || 'Failed to initialize Boostify profile'
      });
    }
  });
