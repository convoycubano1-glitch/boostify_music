...
export function registerRoutes(app: Express): Server {
  ...
  // Registrar rutas directas para Kling API (sin autenticación)
  // Al definirlas directamente aquí evitamos problemas potenciales con el montaje del router
  app.post('/api/kling/try-on/start', async (req, res) => {
    try {
      ...
      if (!PIAPI_API_KEY) {
        ...
        return res.status(500).json({
          success: false,
          error: 'API Key de PiAPI no configurada'
        });
      }

      // Realizar la llamada a la API de Kling
      const response = await axios.post('https://api.piapi.ai/api/v1/task', klingRequest, {
        headers: {
      ...
      // Procesamiento mejorado para respuestas de Kling API
      if (response.data) {
        console.log('Respuesta completa de Kling API:', JSON.stringify(response.data, null, 2));
        
        // Extraer el task_id correctamente de la estructura de respuesta anidada
        let taskId = null;
        
        // Primera opción: directamente en la raíz
        if (response.data.task_id) {
          taskId = response.data.task_id;
        } 
        // Segunda opción: dentro de un objeto data
        else if (response.data.data && response.data.data.task_id) {
          taskId = response.data.data.task_id;
        }
        // Tercera opción: buscar en cualquier lugar
        else {
          // Buscar recursivamente el task_id en cualquier parte del objeto
          const findTaskId = (obj) => {
          ...
          taskId = findTaskId(response.data);
        }
        
        if (taskId) {
          console.log(`✅ Try-On iniciado con éxito: ${taskId}`);
          return res.json({
            success: true,
            taskId: taskId
          });
        } else {
          // No se encontró task_id en ninguna parte de la respuesta
          console.error('❌ Error en respuesta de Kling - No se encontró task_id:', response.data);
          return res.status(500).json({
            success: false,
            error: 'No se encontró ID de tarea en la respuesta',
            details: response.data
          });
        }
      } else {
        // Error en respuesta - respuesta vacía
        console.error('❌ Error en respuesta de Kling - Respuesta vacía');
        return res.status(500).json({
          success: false,
          error: 'Respuesta vacía de Kling API'
        });
      }
    } catch (error: any) {
  ...
  // Ruta para verificar estado del proceso
  app.post('/api/kling/try-on/status', async (req, res) => {
    try {
      ...
      if (response.data) {
        // Comprobar si el status está directamente en la respuesta o dentro de un objeto data
        const responseObj = response.data.data || response.data;
        ...
        if (status === 'success') {
          ...
          // Extraer imágenes de donde sea que estén
          const images = responseObj.output?.images || 
                        responseObj.images || 
                        (responseObj.output && Array.isArray(responseObj.output) ? responseObj.output : []);
          ...
          // Tarea en progreso (pending o cualquier otro estado)
          return res.json({
            success: true,
            status: 'processing'
          });
        ...
        console.error('❌ Respuesta inesperada de Kling API:', response);
        return res.status(500).json({
          success: false,
          error: 'Respuesta inesperada de Kling API',
          details: response.data || 'Sin datos en la respuesta'
        });
      }
    } catch (error: any) {
