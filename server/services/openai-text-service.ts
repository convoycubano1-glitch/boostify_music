/**
 * Servicio de generación de texto con OpenAI GPT-4o
...
async function generateTextWithOpenAI(prompt: string, maxTokens: number = 8192): Promise<string> {
  try {
    console.log('🔑 Generando texto con OpenAI GPT-4o...');
    ...
    const response = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('OpenAI API timeout después de 60 segundos')), 60000)
      )
    ]);
    ...
    console.log('✅ Generación de texto exitosa con OpenAI');
    return content;
  } catch (error: any) {
...
Format as professional AI-powered consulting advice with clear, actionable insights.`
    ...
    console.log('📄 Generando documento profesional con OpenAI GPT-4o...');
