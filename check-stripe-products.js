...
async function main() {
  ...
  // Si no hay productos, ofrecemos crearlos
  if (products.length === 0) {
    ...
    readline.question('\n¿Quieres crear los productos de suscripción? (s/n): ', async (answer) => {
      if (answer.toLowerCase() === 's') {
        await createSubscriptionProducts();
        console.log('\nProductos creados. Vuelve a ejecutar este script para ver los IDs.');
      } else {
        console.log('No se crearon productos. Tendrás que crearlos manualmente desde el dashboard de Stripe.');
      }
      readline.close();
    });
  } else {
